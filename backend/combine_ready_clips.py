...
import os
import config
from analyzer import combine_clips_with_overlay_and_subs
from utility import extract_video_id, time_to_seconds

def get_subs_for_clip(gc, video_id, rank):
    """
    {video_id}_subsシートから指定rankの字幕情報を取得
    Returns: [{'start': float, 'end': float, 'text': str}, ...]
    """
def get_subs_for_clip(subs_values, rank):
    """
    subs_values（3行目以降のリスト）から指定rankの字幕情報を抽出
    Returns: [{'start': float, 'end': float, 'text': str}, ...]
    """
    subs_list = []
    for row in subs_values:
        try:
            # 0: rank, 1: start, 2: end, 3: text
            if str(row[0]) == str(rank):
                text = row[3].replace('+', '\\N').replace('\n', '\\N')
                subs_list.append({
                    'rank': rank,
                    'start': float(row[1]),
                    'end': float(row[2]),
        ...
        for i, row in enumerate(all_data):
            if row.get("分析フラグ") == "TRUE" and row.get("結合フラグ") != "TRUE":
                ...
                try:
                    ...
                    ready_flag = video_sheet.acell("B1").value  # B1に準備完了フラグがある想定
                    if ready_flag == "TRUE" :
                        video_subs_sheet = gc.open_by_key(config.GOOGLE_SHEETS_ID).worksheet(f"{video_id}_subs")
                        print(f"結合中: {video_id} ({video_url})")
                        # クリップ情報取得（3行目以降）
                        ...
                        for clip_row in clip_values:
                            try:
                                ...
                                combine_order = clip_row[16]
                                if combine_checked and combine_order:
                                    rank_val = int(clip_row[0])
                                    subs_values = video_subs_sheet.get_all_values()[2:]  # subsシート3行目以降を一度だけ取得
                                    subs_info = get_subs_for_clip(subs_values, rank_val)
                                    clips.append({
                                        ...
                                        "comment": clip_row[14],
                                        "group_id": group_id,
                                        "combine_order": int(combine_order),
                        ...
                        # 結合順でソート
                        clips = sorted(clips, key=lambda x: x["combine_order"])
                        print(f"結合中: {video_id} クリップ数: {len(clips)}")
                        combine_clips_with_overlay_and_subs(video_url=video_url, group_id=group_id, combine_order=clips)
                        print("結合完了")
                    else:
                        print(f"準備完了フラグがOFF: {video_id} (A2: {ready_flag})")
                except Exception as e:
                    print(f"結合失敗: {e}")
            else:
